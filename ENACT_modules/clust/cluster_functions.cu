...
__global__ void clustering_rcda(const float* Keys_rows, const float* Keys_cols, const float* Values, const float* entropy, const int* entropy_step, const int* start_inds, const int* sizes, const int num_clusters, const int feature_dims, float* Keys_rows_cl, float* Keys_cols_cl, float* Values_cl){
    ...
    if (regions<num_clusters && id_feat<feature_dims){
        ...
        Keys_cols_cl[regions*feature_dims+id_feat]=sum_k_c/sum_exp;
        Values_cl[regions*feature_dims+id_feat]=sum_v/sum_exp;
    }
}
...
__global__ void grad_clustering_rcda(const float* grad_Keys_rows_cl, const float* grad_Keys_cols_cl, const float* grad_Values_cl, const float* Keys_rows, const float* Keys_cols, const float* Values, const float* entropy, const int* entropy_step, const int* start_inds, const int* sizes, const int num_clusters, const int feature_dims, float* grad_Keys_rows, float* grad_Keys_cols, float* grad_Values, float* grad_entropy){
...
vector<torch::Tensor> enact_cluster_forward_rcda(torch::Tensor Keys_rows, torch::Tensor Keys_cols, torch::Tensor Values, torch::Tensor Entropy, torch::Tensor Entropy_step, torch::Tensor start_inds, torch::Tensor region_lengths){
    ...
    torch::Tensor Keys_rows_cl = torch::zeros({region_lengths.size(0), Keys_rows.size(1)}, Keys_rows.options());
    ...
    dim3 numBlocks(n_blocks_reg, n_blocks_ft);
    dim3 threadsPerBlock(n_threads_reg, n_threads_ft);
    clustering_rcda<<<numBlocks, threadsPerBlock>>>(Keys_rows.data_ptr<float>(), Keys_cols.data_ptr<float>(), Values.data_ptr<float>(), Entropy.data_ptr<float>(), Entropy_step.data_ptr<int>(), start_inds.data_ptr<int>(), region_lengths.data_ptr<int>(), region_lengths.size(0), Keys_rows.size(1), Keys_rows_cl.data_ptr<float>(), Keys_cols_cl.data_ptr<float>(), Values_cl.data_ptr<float>());

    return{
        Keys_rows_cl, Keys_cols_cl, Values_cl
    };
}
...
vector<torch::Tensor> enact_cluster_backward_rcda(torch::Tensor grad_Keys_rows_cl, torch::Tensor grad_Keys_cols_cl, torch::Tensor grad_Values_cl, torch::Tensor Keys_rows, torch::Tensor Keys_cols, torch::Tensor Values, torch::Tensor Entropy, torch::Tensor Entropy_step, torch::Tensor start_inds, torch::Tensor region_lengths){
    ...
    torch::Tensor grad_Keys_rows = torch::zeros({Entropy.size(0),   grad_Keys_rows_cl.size(1)},   grad_Keys_rows_cl.options());
    torch::Tensor grad_Keys_cols = torch::zeros({Entropy.size(0),   grad_Keys_cols_cl.size(1)},   grad_Keys_cols_cl.options());
    torch::Tensor grad_Values    = torch::zeros({Entropy.size(0),   grad_Values_cl.size(1)},         grad_Values_cl.options());
    torch::Tensor grad_entropy   = torch::zeros({Entropy.size(0),   grad_Values_cl.size(1)},                Entropy.options());
