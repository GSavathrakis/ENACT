...
__global__ void attention(const float* attn_w, const float* val, const int n_heads, const int batch_size, const int* start_inds, const int* sizes, const int row_attn_w, const int row_val, const int col_val, float* attn){

    int bs_n_heads = blockIdx.z;
    int id_attn_w = blockIdx.x*blockDim.x + threadIdx.x;
    int id_val = blockIdx.y*blockDim.y+threadIdx.y;

    if (bs_n_heads<batch_size*n_heads && id_attn_w<row_attn_w && id_val<col_val){
        float sum=0.;
        for (int s=start_inds[bs_n_heads];s<start_inds[bs_n_heads]+sizes[bs_n_heads];s++){
...
__global__ void grad_v(const float* soft_attn_ws, const float* grad_outp, const int n_heads, const int batch_size, const int* start_inds, const int* sizes, const int col_attn_w, const int row_grad_out, const int col_grad_out, float* grad_val){

    int bs_n_heads  = blockIdx.z;
    int id_attn_ws  = blockIdx.x*blockDim.x+threadIdx.x;
    int id_grad_out = blockIdx.y*blockDim.y+threadIdx.y;

    if (bs_n_heads<n_heads*batch_size && id_attn_ws>=start_inds[bs_n_heads] && id_attn_ws<start_inds[bs_n_heads]+sizes[bs_n_heads] && id_grad_out<col_grad_out){
        float sum=0.;
...
__global__ void grad_q(const float* grad_attn_ws, const float* keys, const int n_heads, const int batch_size, const int* start_inds, const int* sizes, const int row_grad_attn_ws, const int row_keys, const int col_keys, float* grad_queries){

    int bs_n_heads = blockIdx.z;
    int id_row_grad_attn = blockIdx.x*blockDim.x+threadIdx.x;
    int id_col_keys = blockIdx.y*blockDim.y+threadIdx.y;

    if (bs_n_heads<batch_size*n_heads && id_row_grad_attn<row_grad_attn_ws && id_col_keys<col_keys){
        float sum=0.;
        for (int s=start_inds[bs_n_heads];s<start_inds[bs_n_heads]+sizes[bs_n_heads];s++){
...
__global__ void grad_k(const float* grad_attn_ws, const float* queries, const int n_heads, const int batch_size, const int* start_inds, const int* sizes, const int col_grad_attn_ws, const int col_queries, const int row_queries, float* grad_keys){

    int bs_n_heads = blockIdx.z;
    int id_col_grad_attn_ws = blockIdx.x*blockDim.x+threadIdx.x;
    int id_col_queries = blockIdx.y*blockDim.y+threadIdx.y;

    if (bs_n_heads<batch_size*n_heads && id_col_grad_attn_ws>=start_inds[bs_n_heads] && id_col_grad_attn_ws<start_inds[bs_n_heads]+sizes[bs_n_heads] && id_col_queries<col_queries){
        float sum=0.;
